...
//   Delete Course
  ...
  if (!result.isConfirmed) return;
    ...
    try {
        await axios.delete(`${URL}/deleteprocourse/${id}`);
        let updatedcourses= allcourses.filter(course=>course._id !== id)
        setallcourses(updatedcourses)
        Swal.fire("Course deleted successfully!!");
    } catch (error) {
        Swal.fire("Failed to delete Course!!");
        console.log(error);
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full px-20 md:px-0 mt-8 md:mt-10 justify-center'>
        {filterdata.length>0 ? (
          filterdata.map((course)=>(
            <div
              className='w-full h-64 md:h-80 relative overflow-hidden rounded-2xl group'
              key={course._id}
            >
                <RiDeleteBinFill onClick={()=>handledelete(course._id)} className='absolute z-20 right-5 top-5 text-3xl text-red-500 cursor-pointer'/>
              <img src={`${URL}/uploadprocourse/${course.courseicon}`} alt="" onClick={()=>handleview(course._id)} className='absolute m-3 md:m-5 w-10 h-10 md:w-12 md:h-12 cursor-pointer z-10 rounded-full'/>
