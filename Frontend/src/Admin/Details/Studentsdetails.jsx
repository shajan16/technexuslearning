...
const Studentsdetails = () => {
  ...
  let handledelete= async (id)=>{
    ...
    title: "Are you sure?",
    text: "This Student Details will be permanently deleted!",
    icon: "warning",
  ...
  if (!result.isConfirmed) return;
    ...
    try {
        ...
        let updatedstudent= allstudents.filter(student=>student._id !== id)
        setallstudents(updatedstudent)
        Swal.fire("Student Details deleted successfully!!");
    } catch (error) {
        Swal.fire("Failed to delete Student Details!!");
    }
  }
  ...
  return (
   ...
   <div className='px-4 md:px-16 absolute text-3xl md:text-5xl'>
    ...
    <div className='flex flex-col justify-center items-center mt-15 mb-50 mx-10'>
          <h1 className='text-3xl md:text-6xl font-bold text-center'>Students Details</h1>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full px-5 md:px-0 mt-8 md:mt-10 justify-center'>
            ...
            <div className='w-full h-64 md:h-80 relative overflow-hidden cursor-pointer rounded-2xl group' key={student._id}>
              <RiDeleteBinFill onClick={()=>handledelete(student._id)} className='absolute z-20 right-5 top-5 text-3xl text-red-500 cursor-pointer'/>
